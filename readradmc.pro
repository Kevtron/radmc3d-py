...
iformat=0
readf,funit,iformat
if iformat ne 1 and iformat ne 2 then begin
...
return,{nx:nx,ny:ny,nrfr:nf,sizepix_x:sizepix_x,sizepix_y:sizepix_y,$
        image:image,flux:flux,x:x,y:y,lambda:lambda,radian:radian}
end
...
pro doimage,incl=incl,phi=phi,npix=npix,sizecm=size,sizeau=sizeau,$
            ...
            image=image,dcatch=dcatch,options=options, _EXTRA=_extra
makeimage,incl=incl,phi=phi,npix=npix,sizecm=size,sizeau=sizeau,$
              ...
              secondorder=secondorder,dcatch=dcatch
image=readimage()
...
;--------------------------------------------------------------------------
function read_levelpop,molecule,mirror=mirror,ilayer=ilayer,fill=fill
  close,1
  grid=read_amr_grid(mirror=mirror)
  ...
  close,1
  if n_elements(ilayer) eq 0 then ilayer=-1
  a = {grid:grid,pop:pop,ilayer:ilayer,nlev:nlev,ilev:ilev,molecule:molecule}
  if keyword_set(fill) and grid.gridstyle eq 10 then begin
